...
export async function POST(request: Request) {
  try {
    ...
    if (error.code === 'ER_DUP_ENTRY') {
      ...
      const duplicateValue = match ? match[1] : 'the provided value';
      
      return NextResponse.json({ message: `Room number "${duplicateValue}" already exists.` }, { status: 409 });
    }
    // ----------------------

    console.error('API POST /api/dashboard/rooms Error:', error);
